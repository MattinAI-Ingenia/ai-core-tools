...
import type { StreamEvent, LightRAGGraphData } from '../types/streaming';
...
export interface Agent {
  ...
  // LightRAG-specific: query mode (skill-routed | local | global | hybrid | mix | naive | bypass)
  lightrag_query_mode?: string | null;
  lightrag_query_modes?: string[];
  // RAG retrieval config
  rag_k?: number;
  rag_k_mode?: 'fixed' | 'per_100_chunks';
...
class ApiService {
  ...
  async getMiddlewares(appId: number): Promise<Middleware[]> {
    return this.request(`/internal/apps/${appId}/middlewares/`) as Promise<Middleware[]>;
  }

  async getMiddleware(appId: number, middlewareId: number): Promise<Middleware> {
    return this.request(`/internal/apps/${appId}/middlewares/${middlewareId}`) as Promise<Middleware>;
  }
  ...
  /** Poll an inference job: progress while running, the proposed types when done. */
  async getEntityTypeInferenceStatus(appId: number, siloId: number, jobId: string) {
    return this.request(
      `/internal/apps/${appId}/silos/${siloId}/lightrag/infer-entity-types/${jobId}`
    );
  }
  ...
  ): Promise<{ results: SearchResult[]; lightrag_graph?: LightRAGGraphData | null }> {
  ...
  ): Promise<{ data: { results: SearchResult[]; lightrag_graph?: LightRAGGraphData | null }; serverMs: number | null }> {

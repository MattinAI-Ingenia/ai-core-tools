...
def upgrade() -> None:
    # 1. AppCollaborator.invited_by must be nullable before a SET NULL FK is meaningful.
    op.alter_column('AppCollaborator', 'invited_by', nullable=True)

    # 2. Drop the three original NO ACTION foreign keys (autogenerated names).
    ...
    # 3. Recreate them with ON DELETE SET NULL and explicit, stable names so the
    #    downgrade can reference them unambiguously.
...
def downgrade() -> None:
    # 1. Drop the SET NULL foreign keys.
    ...
    # 2. Restore the original NO ACTION foreign keys with their original names.
    op.create_foreign_key(
    ...
    # 3. Restore AppCollaborator.invited_by to NOT NULL.
    #    NOTE: this step FAILS if any AppCollaborator row has invited_by = NULL,
    #    which happens once a user that had invited collaborators has been deleted
    #    (the SET NULL rule nulled those rows). This is irreversible by design —
    #    backfill invited_by on the affected rows before downgrading.

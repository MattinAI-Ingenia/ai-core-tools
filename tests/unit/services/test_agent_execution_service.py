...
class TestCheckpointRetryRecovery:
    ...
    async def test_retries_from_prior_checkpoint_without_deleting_thread(self):
        ...
        create_agent = AsyncMock(return_value=(chain, None, None, None))
...
# ---------------------------------------------------------------------------
# Regression: _execute_agent_async must not reference an undefined
...
# temp_silo_ids is now a real parameter defaulting to None, so callers that
# don't supply one pass it through as None rather than omitting it.
# ---------------------------------------------------------------------------


class TestExecuteAgentAsync:
    @pytest.mark.asyncio
    async def test_does_not_raise_nameerror_and_defaults_temp_silo_ids_to_none(self):
        svc, _ = make_service()
        ...
        assert mock_create_agent.call_args.kwargs["temp_silo_ids"] is None
...
class TestResetAgentConversationSandboxOwnership:
    ...
    async def test_destroys_sandbox_for_owned_conversation(self):
        ...
        # Two independent teardown paths call destroy() here — the explicit
        # ownership-checked block above, and ConversationService.delete_conversation
        # further down (not mocked in this test) doing its own unconditional
        # sandbox cleanup. Harmless (same key, idempotent destroy), but real —
        # assert on the key both calls use rather than a single call.
        assert mock_sss.destroy.call_args_list
        for call in mock_sss.destroy.call_args_list:
            assert call.args[0] == "conv_1_42"
